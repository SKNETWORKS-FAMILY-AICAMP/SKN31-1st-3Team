...
| 이름 | 회고 내용 |
| :---: | :--- |
| **박동관** | 프로젝트를 큰 문제 없이 마무리해서 다행이고 뿌듯했습니다.<br>팀 프로젝트가 익숙하지 않은 상태에서 GitHub로 협업을 처음 해봐서 초반에는 조금 헤맸던 것 같습니다.<br>그래도 진행하면서 점점 익숙해졌고, 협업의 중요성을 직접 느낄 수 있었습니다.<br>또 데이터를 직접 다뤄보면서 데이터의 중요성과 가치에 대해서도 많이 배우는 시간이었습니다. |
| **김봉남** | 전기차라는 새로운 도메인의 부족함을 채우기 위해 자료 조사에 오랜 시간을 들였습니다.<br>
적합한 전기차 충전요금 데이터셋이 없어 한국전력공사의 전기차 충전 판매 실적 데이터를 기반으로 단가를 직접 역산하고 전처리하여 분석용 데이터를 산출해낸 과정이 가장 힘들었고, 그만큼 기억에 남는 것 같습니다.<br>
특히 결측치 처리와 이상치를 어떻게 정제하면 좋을지 많이 고민했고, 데이터의 정합성을 최대한 확보하기 위해 노력했던 과정이 큰 배움이 되었습니다.<br>
향후에는 계절별, 장소별 요금 변동성을 반영한 요금 모델링을 해보고 싶다는 생각을 하고 있습니다.<br>
또한 저렴한 유지비와 보조금 혜택뿐만 아니라 높은 보험료, 배터리 교체 비용, 상대적으로 비싼 차량 가격까지 반영한 진정한 TCO(총소유비용) 비교 모델을 구축해보고 싶습니다.<br>
짧은 시간 동안 많은 것을 배울 수 있었던 의미 있는 경험이었습니다. |
| **이영창** | 직접 sql에 칼럼명을 정하고 원하는 데이터를 표형식으로 올려보니 데이터를 불러올 때 어떻게 가져와야 내가 원하는 정보를
가져올 수 있는지 감이 잡혔고. <br> 대분류에서 하나 하나 내려가보면서 크롤링도 해보니 웹페이지에 정보가 등록되어있는 순서를 알게 돼서 감이 안잡혔던 수업내용의 이해가 이루어졌던 시간 이었습니다. |
| **박종현** | 팀원들이 열심히 참여하는 열성적인 분위기 덕분에 나도 자연스럽게 따라가며 프로젝트를 끝까지 완주할 수 있었다.<br>
하지만 지도 페이지에서의 로딩 문제를 해결하지 못한 점은 아쉬움으로 남는다.<br>
데이터 용량을 줄이거나 처리 방식을 개선했다면 더 나은 결과를 만들 수 있었을 것 같다.<br>
FAQ 페이지 역시 사용자 편의성 등 더 나아질 수 있는 부분들이 보여 완성하기 전 고려하지 못한 부분들이 아쉽게 느껴진다.<br>
아쉬운 부분도 있었지만, 전체적으로는 즐겁게 참여했고 많이 배울 수 있었던 경험이었다. |
