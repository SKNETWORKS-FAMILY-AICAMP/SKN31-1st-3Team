...
그래도 진행하면서 점점 협업 방식에 익숙해졌고, 코드 충돌이나 브랜치 관리에 대한 이해도도 높아졌습니다.<br>
이번 프로젝트에서는 단순히 화면을 구현하는 것을 넘어서, 데이터 수집부터 가공, 시각화까지의 전체 흐름을 직접 경험할 수 있었습니다.<br>
외부 사이트에서 유가 및 전기요금 데이터를 크롤링하여 수집하고, 이를 pandas DataFrame 형태로 변환한 뒤 결측치 처리 및 형 변환 등 전처리 과정을 거쳐 분석 가능한 형태로 정제했습니다.<br>
이후 정제된 데이터를 기반으로 전기차와 내연차의 비용을 비교할 수 있는 로직을 구현하고, 사용자가 직관적으로 이해할 수 있도록 시각화하는 데 집중했습니다.<br>
특히 단순한 수치 나열이 아니라, 절감률과 같은 지표를 계산하여 실제 의사결정에 도움이 되는 정보를 제공하려고 했습니다.<br>
또한 그래프 구성에 있어서도 전기요금 데이터를 강조하고, 변동성을 비교할 수 있는 시각 요소를 추가하는 등 사용자 입장에서 해석하기 쉽도록 개선을 반복했습니다.<br>
이번 경험을 통해 데이터 엔지니어링, 데이터 분석의 기초적인 흐름을 직접 구현해볼 수 있었고, 단순한 기능 구현이 아닌 “사용자에게 어떤 정보를 어떻게 전달할 것인가”에 대해 고민해볼 수 있었던 점이 가장 큰 수확이었습니다.<br>
